    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {shouldShowNavbar && (
        <nav className="sticky top-0 z-50 bg-white shadow-sm border-b">
          <div className="max-w-8xl mx-auto px-10 lg:px-4">
            <div className="flex justify-between h-[87px]">
              <div className="flex items-center">
                <Link to="/" className="flex flex-col items-start">
                  <h1 className="text-2xl font-bold bg-gradient-to-r from-[#0f9d58] to-[#34d399] bg-clip-text text-transparent">Edu Bloom Zone</h1>
              ...
              <div className="hidden md:flex items-center space-x-4">
                <div className='flex items-center gap-2'>
                  {roleFromUrl === 'student' && points && (
                    <span className='flex gap-2 items-center rounded-full px-4 py-[6px] text-white text-sm bg-gradient-to-r from-[#17A64B] to-[#1BC75A]'>
                ...
                {navigation.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.name}
                      to={item.href}
                      className={`flex items-center space-x-4 px-4 py-[10px] rounded-md text-xs font-semibold transition-colors ${
                        isLogout 
                        ? 'bg-[#F9FAFB] border hover:bg-[#FAA832]'
                        : item.current
